...
def compose_instance_v5(
    ...
    input_str = (
        task_desc + f" </s> Time: {step_id}; Score: {int(recent_scores[-1]*100)}; </s> "
    )
    ...
    for obs, action, reward in zip(
        recent_obs[-10:], recent_actions[-10:], recent_reward[-10:]
    ):
...
def compose_instance_v4(
    ...
    """
    Composes a formatted input string for a text-based game agent, combining game state
    and history into a structured prompt.

    The function creates a prompt that includes:
    - Task description
    - Current time step and score
    - Recent action history (last 10 actions) with rewards and observations
    - Current environment description
    - Current inventory contents
    - List of visited rooms (if any)

    The output is formatted with special tokens (</s>) as separators and cleaned of
    unnecessary descriptive text.

    Args:
        mode (str): Must be "fast_system"
        step_id (int): Current time step
        task_desc (str): Description of the current task/goal
        returns_to_go (float): Expected returns (unused)
        curr_action (str): Current action
        curr_obs (str): Current observation
        inventory (str): Current inventory contents
        look (str): Description of current environment
        prev_action (str): Previous action
        prev_obs (str): Previous observation
        objects (list): Available objects (unused)
        places (list): List of visited rooms
        recent_actions (list): History of recent actions
        recent_obs (list): History of recent observations
        recent_scores (list): History of scores
        recent_reward (list): History of rewards

    Returns:
        tuple: (input_str, label) where input_str is the formatted prompt and
               label is the sanitized current action
    """
...
def compose_instance_v3(
    ...
    ind = 10
    for obs, action, reward in zip(
        recent_obs[-5:], recent_actions[-5:], recent_reward[-5:]
    ):
        input_str += f" <extra_id_{ind}> {action} (+{int(reward*100)}) --> {obs} | "
        ind -= 1
    input_str += " </s> "
...
def compose_instance_v1_1(
    ...
    if step_id == 1:
        ...
        prev_obs = curr_obs
    input_str += (
        "The previous observation: " + prev_obs + " </s> "
    )  # TODO: it was actually a bug but it worked great....
    if curr_obs.strip() != look.strip():
        input_str += "Current observation: " + curr_obs + " </s> "
    else:
        input_str += "Current observation: the same as current environment. </s> "
    ...
    if recent_actions:
        input_str += (
            "Recent actions: " + ", ".join(recent_actions[-10:]) + " </s> "
        )  # TODO: 5 or 10? I forgot
    input_str = sanitizeStr(input_str)
    input_str = input_str.replace("(that is open)", "")
    input_str = input_str.replace("(containing nothing)", "")
    label = sanitizeStr(label)
...
def recover_action(formalized_action):
    conversion_dict = [
        ...
        {
            "format_str": "open door to {}",
            "pattern": r"^OPEN_DOOR\((.+)\)",
            "num_args": 1,
        },
        {
            "format_str": "close door to {}",
            "pattern": r"^CLOSE_DOOR\((.+)\)",
            "num_args": 1,
        },
        ...
        {
            "format_str": "pour {} into {}",
            "pattern": r"^POUR\((.+), (.+)\)",
            "num_args": 2,
        },
        {
            "format_str": "dunk {} into {}",
            "pattern": r"^DUNK\((.+), (.+)\)",
            "num_args": 2,
        },
...
def formalize_action(action):
    conversion_dict = [
        ...
        {
            "pattern": r"^open door to (.+)",
            "format_str": "OPEN_DOOR({})",
            "num_args": 1,
        },
        {
            "pattern": r"^close door to (.+)",
            "format_str": "CLOSE_DOOR({})",
            "num_args": 1,
        },
