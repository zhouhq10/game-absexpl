...
class Dungeon {
  constructor(config = {}) {
    ...
    this.rooms = [];
    this.usePredefined = config.usePredefined

    if (this.usePredefined) {
      this.predefinedRooms = config.predefinedRooms;
      this.predefinedTiles = config.predefinedTiles;
      this.visualTiles = config.predefinedVisualTiles;
    } else {
      const rooms = config.rooms;
      rooms.width = Object.assign({}, config.rooms.width, rooms.width);
      rooms.height = Object.assign({}, config.rooms.height, rooms.height);
      rooms.maxArea = rooms.maxArea 
      rooms.maxRooms = rooms.maxRooms 
  ...
  generateFromPredefinedRooms() {
    // Use predefined room data to create Room instances
    if (this.predefinedRooms && this.predefinedRooms.length > 0) {
      for (const roomData of this.predefinedRooms) {
        const room = new Room(roomData.width, roomData.height);
        room.setPosition(roomData.x, roomData.y);
        
        // Copy the tiles data if available
        if (roomData.tiles) {
          room.tiles = JSON.parse(JSON.stringify(roomData.tiles));
        }
  ...
  getMappedTiles(tileMapping = {}) {
    ...
    return this.tiles.map(row =>
      row.map(tile => {
        if (tile === TILE_MAPPING.EMPTY) return tileMapping.empty;
        else if (tile === TILE_MAPPING.WALL) return tileMapping.wall;
        else if (tile === TILE_MAPPING.FLOOR) return tileMapping.floor;
        else if (tile === TILE_MAPPING.DOOR) return tileMapping.door;
      })
    );
  }
