import TILES from "./marple-tile-mapping.js";
// Insert trial name to visualize below
export let gridName = "example1_before";

let grid;
import(`../marple_trials/${gridName}.js`).then(module => {
  grid = module.default;
}).catch(error => {
  console.error("Failed to load the grid:", error);
});

// Defines a new class MarpleScene that extends Phaser.Scene. 
// This class represents a scene in the Phaser game framework.
export default class MarpleScene extends Phaser.Scene {
  ...
  create() {
    ...
    // CREATE BLANK MAP
    const map = this.make.tilemap({ // Creates a new tilemap object (function of Phaser)
      tileWidth: 32,
      tileHeight: 32,
    ...
    // Tilesets are collections of tiles that can be used to create more complex patterns.
    const tileset = map.addTilesetImage("tiles", null);

    // Create a new, empty layer in the tilemap. 
    // Layers are used to organize different types of tiles (e.g., floor, background).
    ...
    this.grid.rooms.initial.forEach(room => {
        ...
        this.floor.fill(TILES.FLOOR[`TOP_${floor_type}`], left + 1, top, width - 1, 1); // Fills a rectangular area with a specific tile.
